...
function regexProfileFallback(html, username = "") {
  ...
  return {
    user: {
      ...
      secUid: str("secUid"),
      // Do not trust region/language from a wide regex window. TikTok can place the
      // logged-in session/app-context close to the viewed profile in the same HTML.
      region: null,
      language: null,
...
function extractJsonLdProfile(html, username = "") {
  const wanted = String(username || "").toLowerCase();
  const source = String(html || "");
  ...
  while ((match = re.exec(source))) {
    try {
      ...
      for (const root of candidates) {
        const entity = root?.mainEntity || root;
        if (!entity || typeof entity !== "object") continue;
        const ids = [entity.alternateName, entity.identifier, entity.username, entity.uniqueId]
          .filter(Boolean)
        ...
        return {
          uniqueId: username,
          nickname: entity.name || null,
          signature: entity.description || null,
          language: typeof entity.knowsLanguage === "string" ? entity.knowsLanguage : null,
        };
      }
    } catch {}
  }
  ...
  try {
    ...
    const onResponse = async (networkResponse) => {
      try {
        ...
        if (!/\/api\/post\/item_list\//.test(url) || capturedPostResponses.length >= 8) return;
        ...
        const captured = { url, status, text, data };
        capturedPostResponses.push(captured);
        resolvePostCapture(captured);
      } catch {
    ...
    let parsed = await page.evaluate((expectedUsername) => {
      ...
      try {
        ...
        const actual = String(user?.uniqueId || "").toLowerCase();
    ...
    const itemAuthor = (item) => item?.author || item?.itemStruct?.author || item?.authorInfo || null;
    const itemBelongsToTarget = (item) => {
      const author = itemAuthor(item);
      const authorUnique = String(author?.uniqueId || author?.unique_id || author?.username || "").toLowerCase();
      const authorSecUid = String(author?.secUid || author?.sec_uid || "");
      if (authorUnique) return authorUnique === String(u.uniqueId || username).toLowerCase();
      if (authorSecUid && u.secUid) return authorSecUid === String(u.secUid);
      return false;
    };
    const requestTargetsUser = (url = "") => {
      try {
        const parsedUrl = new URL(url);
        const secUid = parsedUrl.searchParams.get("secUid") || parsedUrl.searchParams.get("sec_uid");
        return !!secUid && !!u.secUid && secUid === String(u.secUid);
      } catch { return false; }
    };
    const capturedTargetsUser = (captured) => {
      if (!captured) return false;
      if (requestTargetsUser(captured.url)) return true;
      return parseItems(captured.data).some(itemBelongsToTarget);
    };

    const makePostResult = (source, httpStatus, data, text, error = null, requestUrl = "") => {
      const allItems = parseItems(data);
      const trustedRequest = requestTargetsUser(requestUrl);
      const items = allItems.filter((item) => itemBelongsToTarget(item) || (trustedRequest && !itemAuthor(item)));
      ...
      return {
        ...
        rejectedItemCount: Math.max(0, allItems.length - items.length),
        ...
        regionLikeFields,
        error: error || (items.length ? null : (allItems.length
          ? "TikTokin post-listan videot eivät vastanneet haettua käyttäjää."
          : data
    ...
    // 1) Primary path: use only a post-list response that belongs to the searched profile.
    const matchingCapturedPostResponse = capturedPostResponses.find(capturedTargetsUser) || null;
    let postApi = matchingCapturedPostResponse
      ? makePostResult(
          "TikTokin oman sivun verkkovastaus",
          matchingCapturedPostResponse.status,
          matchingCapturedPostResponse.data,
          matchingCapturedPostResponse.text,
          null,
          matchingCapturedPostResponse.url
        )
      : {
          attempted: true,
    ...
    if (!profileHasNoVideos && !postApi.ok && !matchingCapturedPostResponse && u.secUid) {
    ...
    if (videoUrl) {
      ...
      const inspected = await vp.evaluate((expectedUsername) => {
        const el = document.querySelector("#__UNIVERSAL_DATA_FOR_REHYDRATION__");
        if (!el?.textContent) {
          return { hydrationFound: false, videoDetailFound: false, authorMatched: false, locationCreated: null, regionLikeFields: [] };
        }
        ...
        try {
          ...
          const authorUnique = String(item?.author?.uniqueId || item?.author?.unique_id || item?.authorUniqueId || "").toLowerCase();
          const expected = String(expectedUsername || "").toLowerCase();
          const authorMatched = !!item && !!authorUnique && authorUnique === expected;
          const hits = [];
          const seen = new WeakSet();
          ...
          if (authorMatched) walk(item);
          return {
            hydrationFound: true,
            videoDetailFound: !!item,
            authorMatched,
            locationCreated: authorMatched ? (item?.locationCreated ?? item?.location_created ?? null) : null,
            regionLikeFields: authorMatched ? hits : [],
          };
        } catch {
          return { hydrationFound: true, videoDetailFound: false, authorMatched: false, locationCreated: null, regionLikeFields: [] };
        }
      }, u.uniqueId || username);
    ...
    return res.status(200).json({
      ...
      diagnostics: {
        ...
        postApiRejectedItemCount: postApi.rejectedItemCount ?? 0,
        ...
        videoAuthorMatched: video.authorMatched ?? false,
